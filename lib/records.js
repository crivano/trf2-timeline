...
const descrFormaIngresso = (codigo, criteriosPromocao) => {
...
const descrMotivoRemocao = (codigo, motivosRemocao) => {
    const motivoRemocao = motivosRemocao.find(m => m.CodigoMotivoRemocao === codigo)
    const descricao = motivoRemocao ? motivoRemocao.DescricaoMotivoRemocao || `Código: ${codigo}` : `Código: ${codigo}`
    return descricao
}
...
export const getRecords = async(sessionId, codunidade, pMagistrados, pTiposDesignacao, pMotivosDesignacao, pCriteriosPromocao, pMotivosRemocao) => {
    ...
    promocoes.forEach(p => {
        prorem.push({
            ...
            descricao: 'Promoção: ' + descrFormaIngresso(p.CodigoCriterioPromocao, criteriosPromocao),
    ...
    remocoesDe.forEach(p => {
        prorem.push({
            ...
            fim: p.DataFinalExercicio,
            descricao: 'Remoção (de): ' + descrMotivoRemocao(p.CodigoMotivoRemocao, motivosRemocao),
    ...
    remocoesPara.forEach(p => {
        prorem.push({
            ...
            inicio: p.DataRemocao,
            fim: p.DataFinalExercicio,
            descricao: 'Remoção: ' + descrMotivoRemocao(p.CodigoMotivoRemocao, motivosRemocao),
            obs: [p.Observacao1, p.Observacao2].join(' ').trim(),
            doc: p.DocumentoRemocao
        })
